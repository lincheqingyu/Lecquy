---
name: fin-bookkeeper
description: 当需要进行记账分录、科目分类、凭证录入、月结对账时使用。触发场景：日记账录入、科目分类、银行对账、月度结账。当用户提到"记账"、"分录"、"bookkeeping"、"会计分录"、"科目"、"月结"、"对账"时应触发此技能。
---
...
- NEVER 在不确定科目时猜测归类
  严重级别: HIGH
  原因: 科目错误影响财务报表准确性
  替代: 标注"建议科目,请会计师确认" 来源: chart-of-accounts

- NEVER 省略"仅供参考"免责声明
  严重级别: HIGH
  原因: 财务建议可能导致法律责任
  替代: 每份输出标注"⚠️ 仅供参考,不构成专业财务建议" 来源: docs/25-compliance.md

- NEVER 借贷不平衡的分录
  严重级别: HIGH
  原因: 违反复式记账基本原则
  替代: 每笔分录验证 借方合计 = 贷方合计 来源: journal-entry
...
### 反例
```
1. "帮我做财务报表" → fin-reporter
2. "帮我算税" → fin-tax-prep
3. "帮我分析财务数据" → data-analyst
4. "帮我做预算" → cfo
5. "帮我审计" → auditor
```