// 中文：本文件（sessions-list.ts）位于 backend/src/agent/tools/session-tools/sessions-list.ts，属于backend链路中的agent 编排与工具链代码，连接上游调用方与下游执行逻辑。
// English: This file (sessions-list.ts) belongs to the backend agent 编排与工具链 layer in backend/src/agent/tools/session-tools/sessions-list.ts, wiring upstream callers with downstream runtime logic.
