// 中文：本文件（confirmation-broker.ts）位于 backend/src/runtime/confirmation-broker.ts，属于backend链路中的会话运行时代码，连接上游调用方与下游执行逻辑。
// English: This file (confirmation-broker.ts) belongs to the backend 会话运行时 layer in backend/src/runtime/confirmation-broker.ts, wiring upstream callers with downstream runtime logic.
