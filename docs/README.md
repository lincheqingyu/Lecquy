...
- [项目级 / 20260513-7-系统提示词模块再合并与缓存命中优化 决策沉淀.md](./项目级/20260513-7-系统提示词模块再合并与缓存命中优化%20决策沉淀.md)：取代 20260512-2，作为 system prompt 当前权威架构。把 26 子项收敛到 ~8 个物理文件（BASE / TOOLS / AGENTS / SOUL / IDENTITY / USER / MEMORY / MEMORY.summary）+ 末位 `<env>` 块；当前主路线优先适配 OpenAI Chat Completions-compatible API，Anthropic `cache_control` 只属于 Anthropic adapter；MemoryRecall 移出 system 字段，挂在当轮 API user message 内用 `<retrieved_memory priority="low">` 包裹，同时区分用户可见 transcript / API replay transcript / augmentation 记录；mode 降级为 BASE.md 子段 + `<env>` 标志位；删除 `.lecquy/system-prompt/` 前必须先迁移旧 loader 与测试。附消融实验方法论、§11 P0-P3 落地清单、§13 拆分触发条件（按"消融证伪 + token/频率阈值"判断，不按"形态完整"）
