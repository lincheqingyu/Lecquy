...
>
> 最后同步：2026-05-12（CLAUDE.md v0.4）
...
> 最后更新：2026-05-12（v0.4，§1.3 全面对齐 20260512-2 / AGENTS.md "最终架构"）
>
> 修订记录：
...
### 1.3 system prompt 文件体系（最终架构，2026-05-12）

Lecquy 的 system prompt 文件体系**已锁定**。决策来源：[`docs/项目级/20260512-2-系统提示词上下文工程最终取舍 技术规范.md`](./docs/项目级/20260512-2-系统提示词上下文工程最终取舍%20技术规范.md)。
...
├── USER.md              # kira 稳定画像与长期偏好，源文件软上限 4KB
...
└── system-prompt/
    └── *.md             # identity / role / tooling / safety / runtime 模板
```

**注入顺序**（写进 prompt builder，**不要让每个工具自己拼**）：

可缓存前缀必须连续放在最前面，半缓存技能层放在可缓存前缀之后，动态层永远放最后。详见 20260512-2 §6。

1. System：identity template、tooling、tool-call-style、safety、skills index、workspace、documentation、time、runtime、extra instructions、`.lecquy/AGENTS.md`、`.lecquy/TOOLS.md`
2. Mode：simple / manager / worker role directive
3. StartupContext：capability block、`SOUL.md`、`IDENTITY.md`、`USER.md` profile slice、`MEMORY.summary.md`
4. UserPreference：`USER.md` preference slice
5. SkillRuntime：当前命中的 `SKILL.md` 正文和必要 reference 摘要
6. MemoryRecall：从 `memory.db` 召回的相关事实、决策、错误、项目状态
7. LiveTurn：当前用户消息、路由上下文、附件、工具结果、todo 状态
...
**未来拆分触发条件**：只在出现下列任一信号后拆分，不按"看起来更完整"拆分：

- `USER.md` 超过 6KB，或 profile / preference 连续两次被截断
- memory 召回污染，频繁注入无关记忆
- `TOOLS.md` 超过 1500 tokens，且工具纪律和工具教程混杂
- 出现 3 个以上稳定可选 agent，需要自动路由或不同工具权限
- subagent / mode 切换需要 full / minimal / none 三档预算

**AI 助手强约束**（特别给 Codex / Claude Code 看）：

1. 动这一块时**不要重新设计文件体系**——最终结构已锁定，重新设计是浪费工时。
2. 允许的工作只是把这套结构落到代码 / `.lecquy/` 目录里。
3. 如果看到代码里还没落地，那是历史包袱，按此节追上即可。
4. **特别注意**：`MEMORY.md` 不是主记忆库；权威内容在 `memory.db`，常驻 frozen snapshot 在 `MEMORY.summary.md`，召回时按标签过滤注入。
...
3. **上下文工程 + 主循环** —— 7 层 system prompt 完整，缺"历史决策摘要"动态注入点。
...
7. system prompt 文件体系（§1.3）**已经在 20260512-2 钉死了**，不要再去重新设计 6 文件 vs 拆细子目录这种问题——未来拆分条件在 §1.3 里写明了，按触发条件办。
